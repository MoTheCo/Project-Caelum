...
async function _initializeUI() {
    ...
    panel.destroy(); // Entferne die Top-Bar
    overview.destroy(); // Entferne die Aktivitätenübersicht
    reloadThemeResource();
    _loadIcons();
    _loadOskLayouts();
    _loadDefaultStylesheet();
...
// Interaktive Menüs erstellen und hinzufügen
let leftMenu = createEdgeMenu('left', (menu) => {
...
let bottomMenu = createEdgeMenu('bottom', (menu) => {
    // Vorschau aller offenen Fenster
...
let rightMenu = createEdgeMenu('right', (menu) => {
    // Virtuelle Arbeitsbereiche anzeigen
...
let topMenu = createEdgeMenu('top', (menu) => {
    ...
    shutdownButton.connect('clicked', () => {
        global.reexec_self(); // Neustart der Shell
    });
...
function createEdgeMenu(position, contentCallback) {
    ...
    global.stage.add_actor(menu);

    // Definiere Position und Größe des Menüs
    switch (position) {
        case 'left':
            menu.set_position(0, 0);
            menu.set_size(200, global.stage.height);
            break;
        case 'bottom':
            menu.set_position(0, global.stage.height - 100);
            menu.set_size(global.stage.width, 100);
            break;
        case 'right':
            menu.set_position(global.stage.width - 200, 0);
            menu.set_size(200, global.stage.height);
            break;
        case 'top':
            menu.set_position(0, 0);
